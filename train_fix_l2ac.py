...
def main():
    ...
    if args.dataset == 'cifar100':
        ...
        train_labeled_set, train_unlabeled_set, test_set = fix_cifar10.get_cifar10(
            './data/', N_SAMPLES_PER_CLASS, U_SAMPLES_PER_CLASS)

    labeled_trainloader = data.DataLoader(
        train_labeled_set, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, drop_last=True)
    unlabeled_trainloader = data.DataLoader(
        train_unlabeled_set, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, drop_last=True)
    ...
    print('==> Total params: %.2fM' % (sum(p.numel() for p in model.params()) / 1000000.0))

    train_criterion = SemiLoss()
    criterion = nn.CrossEntropyLoss()

    classifier_params = list(map(id, model.fc.params()))
    feat_params = filter(lambda p: id(p) not in classifier_params,
                     model.params())

    optimizer_feat = torch.optim.Adam(feat_params, lr=args.lr)
    optimizer_fc = torch.optim.Adam(model.fc.params(), lr=args.lr)
    ema_optimizer = WeightEMA(model, ema_model, alpha=args.ema_decay)

    meta_net = models.MetaNet(in_dim=num_class, out_dim=num_class, hid_dim=256).cuda()
    optimizer_meta = torch.optim.Adam(meta_net.params(), 1e-4)
    print('==>Total params: %.2fM' % (sum(p.numel() for p in meta_net.params()) / 1000000.0))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == start_epoch or epoch % 10 == 0:
            meta_loader = data.DataLoader(train_labeled_set, batch_size=args.batch_size,
                                          sampler=ClassAwareSampler(train_labeled_set, 10),
...
def train(labeled_trainloader, unlabeled_trainloader, meta_loader, model, meta_net, optimizer_feat, optimizer_fc,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        try:
            inputs_m, targets_m, _ = meta_iter.next()
        except:
            meta_iter = iter(meta_loader)
            inputs_m, targets_m, _ = meta_iter.next()
        ...
        # build meta_model
        meta_model = models.MetaWideResNet(num_classes=num_class).cuda()
        ...
        with torch.no_grad():
            ...
            if args.imb_ratio_l == args.imb_ratio_u:
                pseudo_labels = F.normalize(outputs_u, dim=-1).detach()
        ...
        if batch_idx % 10 == 0:
            ...
            pseudo_labels = F.normalize(all_outputs, dim=-1).detach()
            ...
            meta_optimizer.step(grad=grads)
            del grads

            logits_meta_hat = meta_model(inputs_m)
            loss_meta = F.cross_entropy(logits_meta_hat, targets_m.long())
